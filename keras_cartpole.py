...
class Agent(object):
    def __init__(self, input_size=4, hidden_size=2, gamma=0.95,
        ...
        # placeholders
        self.input_pl = K.placeholder([None, input_size], dtype='float32')
        self.action_pl = K.placeholder([None], dtype='int32')
        self.reward_pl = K.placeholder([None], dtype='float32')
        ...
        hidden_layer = Dense(hidden_size, activation='relu')(self.input_pl)
        #self.output = layers.fully_connected(hidden_layer,
        #                                     action_size,
        #                                     biases_initializer=None,
        #                                     activation_fn=tf.nn.softmax)
        self.output = Dense(action_size, activation='softmax')(hidden_layer)
        # responsible output
        one_hot = tf.one_hot(self.action_pl, action_size)
        responsible_output = K.sum(self.output * one_hot, axis=1)
        self.loss = -K.mean(K.log(responsible_output) * self.reward_pl)
        # training variables
